          ...
          tabPanel(
            ...
            conditionalPanel(
              ...
              conditionalPanel(
                ...
                imageOutput(outputId = "heatmap2", height="100%", width="100%"),
                tags$hr(),
                tags$p(
                  align="center",
...
server <- function(input, output, session) {
  ...
  # heatmap of DGE, which is really the important heat map to have.  IN PROGRESS 
  createHeatmap2 <- function(){
#    dge_list  =  normalizeData()
    dge_list = filterNorm()
    subset = pairwiseTest()
    #dge_list_filtered = dge_list[rownames(subset)]
    logcounts = cpm(dge_list, log=TRUE)
    heatmap(logcounts, main= "test")
  }
